...
class ServerCreateForm(FlaskForm):
    subdomain = StringField('Subdomain', validators=[DataRequired(), Length(min=1, max=20)])
    # name = StringField('Name', validators=[DataRequired()])
    submit = SubmitField('Create')

class CommandSelectForm(FlaskForm):
    command = SelectField('Command', choices=[("ban", "ban"), ("unban", "unban"), ("op", "op"), ("deop", "deop"), ("whitlist add", "whitlist add"), ("whitlist remove", "whitlist remove"), ], render_kw={"class": "border border-black rounded-l-lg bg-white text-black px-2 py-2 focus:outline-none w-1/3 text-md font-bold", "autocomplete":"off"})
